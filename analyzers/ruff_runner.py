...
def run_ruff(content: str, fn: str) -> list:
    if not fn.endswith(".py"):
        return []
    try:
        # FIX: Send code directly through RAM using '-' and '--stdin-filename'
        r = subprocess.run(
        ...
        # Debugging hook just in case Ruff throws an internal error
        if r.stderr.strip() and not r.stdout.strip():
            print(f"Ruff Internal Error: {r.stderr}")
            return []
